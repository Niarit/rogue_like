...
class MainGameEventHandler(EventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[Action]:
        ...
        if key in MOVE_KEYS:
            ...
            self.engine.event_handler = LookHandler(self.engine)  # Might change
...
class AskUserEventHandler(EventHandler):
    ...
    def handle_action(self, action: Optional[Action]) -> bool:
        """Return the main event handler when a valid action is performed"""
...
class InventoryEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        """Render the inventory menu, which displays the items in the inventory, and the letter to select them"""
...
class SelectIndexHandler(AskUserEventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[Action]:
        """Check for key movement or confirm key"""

        key = event.sym
        if key in MOVE_KEYS:
            modifier = 1  # Holding modifier will speed up key movement
            ...
            # Clamp the cursor index to the size of the map.
    ...
    def ev_mousebuttondown(self, event: tcod.event.MouseButtonDown) -> Optional[Action]:
        """Left click confirms the selection"""
